        ...
        for root, _, filenames in os.walk(folder_path):
            for filename in filenames:
                ...
                if checksum_bytes == "ERROR":
                    ...
                    print(f"❌ {send_file_index} 실패: {relative_path}")
                    #continue
                    time.sleep(0.1)
                ...
                if checksum_err == False:
                    # 3. 로컬 파일의 MD5 계산 후 비교
                    local_checksum = calculate_md5(full_path)
                    #print(local_checksum)
                    if local_checksum == checksum_bytes:
                        print(f"✅ {send_file_index} 완료: {relative_path}")
                    else:
                        print(f"⚠️ {send_file_index} 실패: {relative_path}")
...
if __name__ == "__main__":
    ...
    # 폴더 검증 및 없는 파일 전송
    vaidate_files(PORT, FOLDER_TO_SEND)
